...
def takecorp(corp, n_gram_value):
...
corp_words_and_Title = takecorp(igbb, n_gram_value)
...
# Calling the Token-count after Cut-off Function
tokencount_after_cutoff(word_counts2['n2'])
...
for i in range(bootstraplength):
    ...
    if cutoff_freq > 0:
        ...
        boot_new_bag_of_words = []
        for i in range(cut_off_index):
            for k in range(boot_counts.boot_n_raw[i]):
                boot_new_bag_of_words.append(boot_counts.boot_word[i])
        boots_trimmed = pd.DataFrame(boot_new_bag_of_words, columns = ['bootword2'])
        boot_counts2 = boots_trimmed.bootword2.value_counts().reset_index()
        boot_counts2.columns = ['bootword2', 'boot_n2']
        boot_counts2['boot_rank2'] = boot_counts2.boot_n2.rank(ascending=False)
    #words_trimmed = words.word[:cut_off_index].tolist()
    elif cutoff_freq == 0:
    ...
    yvalues = np.log(boot_counts2.boot_n2)
